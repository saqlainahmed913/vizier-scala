  ...
  with BeforeAll
{
  ...
  Fragment.foreach(Seq(
    ...
    Seq(false, true).map { (start, _) }
                    .filter { 
                      case (0, true) => false
                      case _ => true
                    }
  }) { case (start, mockRecursion) =>
    s"Time Results @ $start" >> 
    {
      val START = start
      val OVERRIDE_RECURSION = mockRecursion

      val client =
        TimerUtils.logTime(s"@$start/$mockRecursion: Init Spreadsheet"){
      ...
      TimerUtils.logTime(s"@$start/$mockRecursion: Monitoring Overhead"){
        client.watchAllVisible()
        client.waitForQuiescence()
      }

      val charge = 
        TimerUtils.logTime(s"@$start/$mockRecursion: Init Formulas"){
          ...
          client.watchAllVisible()
          client.waitForQuiescence()
          if(OVERRIDE_RECURSION){
            val r = 
              data.limit(START-1)
                    .agg(functions.sum(
                      data("ExtendedPrice") * 
                        (functions.lit(1.0) - data("Discount")) *
                        (functions.lit(1.0) + data("Tax"))
                    ))
                    .head()
                    .apply(0)
            client.spreadsheet.overlay.update(
              sum_charge(START-1, 1),
              functions.lit(r).expr
            )
            client.spreadsheet.overlay.update(
              sum_charge(START, rowCount),
              Add(
          ...
          // client.resetSubscription()
          client.watchAllVisible()
          client.waitForQuiescence()
          charge
        }

      TimerUtils.logTime(s"@$start/$mockRecursion: Update one"){
        client.spreadsheet.overlay.update(
          ext_price(0),
          Literal(1000.0f)
        )
      ...
      TimerUtils.logTime(s"@$start/$mockRecursion: Update all"){
        client.spreadsheet.overlay.update(
  ...
  class MockClient extends SpreadsheetCallbacks
  {
    ...
    def init(position: Long, viewable: Int = 50) 
    {
      ...
      this.visible = position until (position + viewable)
    ...
    {
      spreadsheet.subscribe(visible.head, viewable);
      watchAllVisible()
    }
    ...
    def waitForQuiescence(secondsToWait: Int = 60)
    {
      var loops = secondsToWait
      synchronized {
        while( !pendingRows.isEmpty && loops > 0 ){ 
          this.wait(1000)
          loops -= 1
        }
      }
      assert(loops > 0, s"Timed out waiting for quiescence after $secondsToWait seconds")
    }

    def checkPending(rows: Seq[Long] = visible): Boolean =
    {
      synchronized {
        // println(s"Checking for pending : ${rows.mkString(", ")}")
        for(row <- rows)
        {
          if(spreadsheet.getFutureRow(row).forall { _.isCompleted })
          {
